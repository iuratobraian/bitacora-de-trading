...
const Dashboard: React.FC = () => {
    ...
    return (
        <div className="p-4 md:p-8 w-full max-w-[96%] 2xl:max-w-[1920px] mx-auto animate-in fade-in slide-in-from-bottom-2 duration-500 pb-24">
            ...
            {/* TOP INTELLIGENCE CENTER */}
            <div className="mb-0 flex flex-col lg:flex-row gap-6">
                <div className="lg:w-[30%] bg-surface border border-border p-6 rounded-3xl flex flex-col justify-center items-center shadow-xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                        <Trophy size={80} className="text-primary" />
                    </div>
                    <p className="text-gray-500 text-[10px] font-black uppercase mb-2 tracking-widest relative z-10">Win Rate Global</p>
                    <h3 className="text-5xl font-black text-white font-mono tracking-tighter relative z-10">{stats.winRate}%</h3>
                    <p className="text-[10px] text-primary font-bold uppercase tracking-wider relative z-10 mt-2">{stats.wins} Wins / {stats.losses} Losses</p>
                </div>
                ...
                <div className="lg:w-[70%] flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-surface border border-border rounded-3xl p-4 shadow-xl">
                        <IntelligenceCard
                            ...
                            subtext={<span>PF Rec: <span className="text-primary font-black">{'>'}1.50</span></span>}
                            icon={Activity}
                        ...
                        <IntelligenceCard
                            ...
                            subtext={<span>Rec Rec: <span className="text-primary font-black">{'>'}1.00</span></span>}
                            icon={TrendingUp}
            ...
            {/* DISCIPLINE & EQUITY SECTION */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">

                {/* SEMÁFORO DISCIPLINA */}
                <div className="md:col-span-4 bg-surface border border-border rounded-3xl p-6 shadow-2xl relative overflow-hidden flex flex-col justify-between">
                ...
                <div className="md:col-span-8 bg-surface border border-border rounded-3xl p-8 shadow-2xl relative overflow-hidden group min-h-[350px]">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center gap-2 bg-black/40 px-3 py-1 rounded-full border border-white/5">
                            <span className="text-[9px] text-gray-500 font-bold uppercase">PnL Realizado:</span>
                            <span className={`text-[10px] font-mono font-black ${stats.totalPnl >= 0 ? 'text-primary' : 'text-loss'}`}>{formatCurrency(stats.totalPnl)}</span>
                        </div>
                    </div>
            ...
            {/* LOWER GRID: CALENDAR AND BREAKDOWN */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                ...
                <div className="md:col-span-8 bg-surface border border-border rounded-3xl p-8 flex flex-col shadow-2xl min-h-[450px] relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl"></div>
                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-6">
                        <List size={14} className="text-primary" /> Desglose Detallado de Activos
                    </h3>

                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                        <table className="w-full text-left border-collapse">
                            <thead className="sticky top-0 bg-surface z-10 text-[10px] font-black uppercase text-gray-500 tracking-wider">
                                <tr>
                                    <th className="pb-4">Activo</th>
                                    <th className="pb-4 text-center">Trades</th>
                                    <th className="pb-4 text-center">WR%</th>
                                    <th className="pb-4 text-right">RR Avg</th>
                                    <th className="pb-4 text-right">PnL Neto</th>
                                </tr>
                            </thead>
                                ...
                                {stats.assetsBreakdown && stats.assetsBreakdown.length > 0 ? (
                                    stats.assetsBreakdown.map((asset: any, idx: number) => (
                                        <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                                            <td className="py-4 font-black text-white text-sm">{asset.symbol}</td>
                                            <td className="py-4 text-center text-gray-400">{asset.count}</td>
                                            <td className={`py-4 text-center font-black text-sm ${Number(asset.winRate) >= 50 ? 'text-primary' : 'text-loss'}`}>{asset.winRate}%</td>
                                            <td className="py-4 text-right text-gray-400 font-bold">{asset.avgRR} R</td>
                                            <td className={`py-4 text-right font-black text-sm ${asset.pnl >= 0 ? 'text-primary' : 'text-loss'}`}>
                                    ...
                                    <tr>
                                        <td colSpan={5} className="py-20 text-center opacity-30">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <span className="text-[10px] font-black uppercase">SIN DATOS DISPONIBLES</span>
                                            </div>
                                        </td>
                                    </tr>
                                )}
            ...
            {/* Sesiones */}
            <div className="bg-surface border border-border rounded-3xl p-8 shadow-2xl mt-2">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
