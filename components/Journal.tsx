...
import { Trash2, Search, X, RefreshCw, FileSpreadsheet, Plus, Landmark, Eraser, Calendar, ArrowUp, ArrowDown, ArrowRight, Settings as SettingsIcon, Check, Table as TableIcon, Hash, CheckCircle2, Sparkles, Wallet, TrendingUp, TrendingDown, AlertCircle, CloudLightning, Maximize2, Edit, Link as LinkIcon, Activity } from 'lucide-react';
                            ...
                            {selectedTrade.type !== TradeType.BALANCE && (
                                <div className="bg-background/40 rounded-xl p-4 grid grid-cols-3 gap-4 border border-white/5">
                                    ...
                                    <div className="flex flex-col justify-center">
                                        <p className="text-[9px] text-gray-500 font-bold uppercase mb-1">TradingView</p>
                                        <button
                                            disabled={!selectedTrade.tradingViewUrl}
                                            onClick={() => window.open(selectedTrade.tradingViewUrl, '_blank')}
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${selectedTrade.tradingViewUrl ? 'bg-primary/20 text-primary hover:bg-primary/30 border border-primary/30' : 'bg-white/5 text-gray-600 border border-white/5 cursor-not-allowed'}`}
                                        >
                                            <Activity size={12} />
                                            {selectedTrade.tradingViewUrl ? 'VER GRÁFICO' : 'OFFLINE'}
                                        </button>
                                    </div>
                                </div>
                            )}
                ...
                <table className="w-full text-left">
                    <thead className="bg-background border-b border-border">
                        <tr className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
                            ...
                            <th className="p-6 text-center">TV</th>
                                    ...
                                    <td className="p-6 text-center">
                                        {!isBalanceOp && (
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    if (trade.tradingViewUrl) window.open(trade.tradingViewUrl, '_blank');
                                                }}
                                                className={`p-2 rounded-lg transition-all ${trade.tradingViewUrl ? 'text-primary bg-primary/10 shadow-lg shadow-primary/10 hover:scale-110' : 'text-gray-700 opacity-20 cursor-default'}`}
                                                title={trade.tradingViewUrl ? "Abrir en TradingView" : "Sin Link de TradingView"}
                                            >
                                                <Activity size={16} />
                                            </button>
                                        )}
