...
async function calculateStats(userId: string): Promise<UserStats> {
  ...
  const wins = tradingTrades.filter((t: Trade) => t.outcome === 'WIN');
  const losses = tradingTrades.filter((t: Trade) => t.outcome === 'LOSS');
  const breakeven = tradingTrades.filter((t: Trade) => t.outcome === 'BREAKEVEN');

  const winsPnl = wins.reduce((sum: number, t: Trade) => sum + (t.pnl || 0), 0);
  const lossesPnl = Math.abs(losses.reduce((sum: number, t: Trade) => sum + (t.pnl || 0), 0));
  ...
  tradingTrades.forEach((t: Trade) => {
    if (!assetStats[t.asset]) {
      assetStats[t.asset] = { count: 0, pnl: 0 };
    }
  ...
  const pnls = tradingTrades.map((t: Trade) => t.pnl || 0);
  ...
  return {
    ...
    totalPnl: tradingTrades.reduce((sum: number, t: Trade) => sum + (t.pnl || 0), 0),
